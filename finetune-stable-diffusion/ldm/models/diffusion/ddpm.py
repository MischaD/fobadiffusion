...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, loss_dict = self.shared_step(batch)# batch["mask"] # not latent at this point
        #start of training setp
...
class LatentDiffusion(DDPM):
    ...
    @torch.no_grad()
    def sample_log(self, cond, batch_size, ddim, ddim_steps, mask=None, x0=None, **kwargs):

        if ddim:
            ddim_sampler = DDIMSampler(self)
            shape = (self.channels, self.image_size, self.image_size)
            samples, intermediates = ddim_sampler.sample(ddim_steps, batch_size,
                                                         shape, cond,mask=mask,x0=x0, verbose=False,  **kwargs)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if sample:
            ...
            with ema_scope("Sampling"):
                x0, c = self.get_input(batch, self.first_stage_key)
