...
class PLMSSampler(object):
    ...
    def sample_with_attention(self,
        ...
        for i, step in enumerate(time_range):
            ...
            outs = self.p_sample_plms(x_t, conditioning, ts, index=index, use_original_steps=False,
    ...
    def plms_sampling(self, cond, shape,
                      ...
                      unconditional_guidance_scale=1., unconditional_conditioning=None, patch_kwargs={}):
        device = self.model.betas.device
        b = shape[0]
        if x_T is None:
        ...
        intermediates = {'x_inter': [], 'pred_x0': [], 'e_t': []}
        time_range = list(reversed(range(0,timesteps))) if ddim_use_original_steps else np.flip(timesteps)
        #time_range = np.array([1, ] * len(timesteps))
        #print(f"Sampling only from last timestep")
        ...
        for i, step in enumerate(iterator):
            ...
            if patch_kwargs is not {} and patch_kwargs.get("stop_prompt") is not None and i < patch_kwargs["stop_prompt"]["at_iter"]:
                cond = patch_kwargs["stop_prompt"]["uc"]
            else:
                cond = original_cond
            ...
            if index % log_every_t == 0 or index == total_steps - 1:
                ...
                intermediates["e_t"].append(abs(e_t.cpu()))
